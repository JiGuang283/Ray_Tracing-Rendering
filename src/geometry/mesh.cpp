...
mesh::mesh(std::vector<shared_ptr<hittable>> faces, double time0,
           double time1, bool build_bvh)
    : triangles(std::move(faces)) {
    if (build_bvh) {
        accelerator =
            make_shared<bvh_node>(triangles, 0, triangles.size(), time0, time1);
    } else {
...
bool mesh::hit(const ray &r, double t_min, double t_max, hit_record &rec,
               RNG &rng) const {
    ...
    for (const auto &shape : shapes) {
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++) {
            ...
            if (use_vertex_normals && n0.length_squared() > 0 &&
                n1.length_squared() > 0 && n2.length_squared() > 0) {
                faces.push_back(make_shared<triangle>(
                    p0, p1, p2, unit_vector(n0), unit_vector(n1),
                    unit_vector(n2), mat, uv0, uv1, uv2, has_uvs));
            } else {
                faces.push_back(make_shared<triangle>(p0, p1, p2, mat, uv0,
                                                      uv1, uv2, has_uvs));
            }
        }
    }
