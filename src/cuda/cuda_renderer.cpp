...
RenderResult render_cuda(const CompiledScene &scene,
    ...
    transport_settings.sample_clamp = static_cast<float>(request.sample_clamp);
    const CudaRenderOutput output = render_wavefront_cuda(
        device_scene.view(), transport_settings, cancel.native_flag());
    ...
    for (std::uint32_t y = 0; y < transport_settings.height; ++y) {
        for (std::uint32_t x = 0; x < transport_settings.width; ++x) {
            ...
            beauty.set_pixel(
                ...
                color(pixel.radiance.x, pixel.radiance.y, pixel.radiance.z),
                pixel.sample_count);
        }
    }
