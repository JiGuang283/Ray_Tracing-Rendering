...
SceneConfig select_scene(int scene_id) {
    ...
    case 17: {// IBL 测试
        ...
        // 添加环境光
        // 注意：把你的 HDR 文件命名为 sky.hdr 放在 exe 同级目录
        ...
        config.aspect_ratio = 16.0 / 9.0;
        config.image_width = 800;
        config.samples_per_pixel = 200; // 面光源噪点多，采样稍微高点
        config.background = color(0, 0, 0);
        ...
        auto ball = make_shared<lambertian>(color(0.7, 0.3, 0.1));
        objects.add(make_shared<sphere>(point3(0, 1, 0), 1, ball));
        
        // 注意：为了让 NEE (Next Event Estimation) 正常工作，
        // 光源不仅要在 config.lights 里，最好也有一个对应的发光几何体在 config.world 里，
        // 这样当相机直接看向光源时，能看到它是亮的。
        // 但简单起见，我们只添加 Light 接口，这样光源是“隐形”的，但能照亮物体。
        
        config.world = make_shared<bvh_node>(objects, 0, 1);

        // 添加一个位于上方的大矩形光源
        // 角点 (-2, 6, -2)，边长 4x4
        auto light = make_shared<QuadLight>(
            point3(-2, 6, -2), 
            vec3(4, 0, 0), 
            vec3(0, 0, 4), 
            color(10, 10, 10) // 强度高一点
        );
