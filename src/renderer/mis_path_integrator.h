...
public:
    ...
    virtual color Li(const ray& r, const hittable& scene,
        ...
        // ★★★ 将成员变量 m_lights 传给核心逻辑 ★★★
        return Li(r, scene, background, m_lights);
    }
    ...
    virtual color
        ...
        for (int depth = 0; depth < m_max_depth; ++depth) {
            ...
            if (!scene.hit(current_ray, 0.001, infinity, rec)) {
                ...
                // 遍历光源列表，寻找环境光 (Non-delta light)
                for (const auto& light : lights) {
                    if (!light->is_delta()) {
                        color env = light->Le(current_ray);
                        if (env.length_squared() > 0) {
                            L += throughput * env;
                            found_env = true;
                            // 通常只有一个环境光，找到后可以直接 break，
                            // 或者你可以累加多个环境光
                            break;
                        }
                    }
            ...
            if (emitted.length_squared() > 0) {
                ...
                if (depth == 0 || specular_bounce) {
                    // 第一次命中或镜面反射后：无 MIS (因为之前的路径没法对光源采样)
            ...
            if (!rec.mat_ptr->sample(rec, wo, bs)) {
                // 如果材质不支持采样（还是旧的 scatter 接口），回退到旧逻辑
                ray scattered;
                color attenuation;
...
private:
    ...
    color
        ...
        if (ls.pdf > 0 && ls.Li.length_squared() > 0) {
            ...
            // 发射阴影光线 (t_min 可以是 0.0，因为已经偏移了)
            ray shadow_ray(offset_origin, ls.wi, 0); // 注意时间参数如果需要 motion blur 要加上
            hit_record shadow_rec;

            // 遮挡测试
            bool in_shadow =
                scene.hit(shadow_ray, 0.0, ls.dist - 1e-3, shadow_rec);
