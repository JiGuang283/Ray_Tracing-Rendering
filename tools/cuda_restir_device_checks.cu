...
CudaReservoirDeviceCheckResult run_cuda_reservoir_device_check() {
    ...
    check_cuda(cudaMemset(device_counts, 0,
    ...
    const DeviceAlgebraResult host = run_host_algebra();
    CudaReservoirDeviceCheckResult result;
    result.trials = trials;
    result.failures = counts[1];
    result.heavy_observed =
        static_cast<double>(counts[0]) / static_cast<double>(trials);
    result.tolerance =
        6.0 * std::sqrt(0.75 * 0.25 / static_cast<double>(trials)) +
        0.001;
    result.algebra_pass =
        algebra.status == 1u && host.status == 1u &&
        algebra.source.sample == host.source.sample &&
        algebra.source.M == host.source.M &&
        near(algebra.source.weight_sum, host.source.weight_sum) &&
        near(algebra.source.selected_target, host.source.selected_target) &&
        near(algebra.source.unbiased_contribution_weight,
             host.source.unbiased_contribution_weight) &&
        algebra.combined.sample == host.combined.sample &&
        algebra.combined.M == host.combined.M &&
        near(algebra.combined.weight_sum, host.combined.weight_sum) &&
        near(algebra.combined.selected_target,
             host.combined.selected_target) &&
        near(algebra.combined.unbiased_contribution_weight,
             host.combined.unbiased_contribution_weight);
    return result;
}
